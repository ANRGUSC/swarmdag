...
import (
	...
	"github.com/tendermint/tendermint/swarmdag/tm_app/monitor"
        ...
        case <-proposeTicker.C:
            ...
            if mm.state == FOLLOW_ANY {
                if mm.checkMembershipChange() {
                    log.Debugf("proposing membership update to view ID %d\n", mm.viewID + 1)
        ...
        case <-rcvdMsg:
            ...
            case FOLLOW_ANY:
                if mlist["PROPOSE"] == 1 && mlist["VIEW_ID"] > mm.membershipList["VIEW_ID"] {
                    log.Debug("ACKing proposal for membership update to", src.String())
